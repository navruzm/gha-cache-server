...
- **publish-chart** — the Helm chart in `charts/gha-cache-server` is repackaged with `mise run helm-package -- $VERSION` and pushed via `mise run helm-push` to `oci://ghcr.io/<owner>/charts`.
